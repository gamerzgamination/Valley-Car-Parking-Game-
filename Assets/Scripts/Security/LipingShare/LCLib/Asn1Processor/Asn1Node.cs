...
namespace LipingShare.LCLib.Asn1Processor
{
	internal class Asn1Node : IAsn1Node
	{
		...
		public byte MaskedTag => (byte)(tag & 0x1Fu);
		...
		public long ChildNodeCount => childNodeList.Count;

		public string TagName => Asn1Util.GetTagName(tag);

		public Asn1Node ParentNode => parentNode;

		public string Path => path;

		public long DataLength => dataLength;

		public long LengthFieldBytes => lengthFieldBytes;
		...
		public string GetText(Asn1Node startNode, int lineLen)
		{
			...
			case 6:
			{
				...
				string oidName = oid.GetOidName(text4);
				text += string.Format("{0,6}|{1,6}|{2,7}|{3} {4} : {5} [{6}]\r\n", dataOffset, dataLength, lengthFieldBytes, GetIndentStr(startNode), TagName, oidName, text4);
				break;
			}
			case 13:
			{
		...
		public string GetLabel(uint mask)
		{
			...
			string text2 = "";
			string text3 = "";
			text3 = (((mask & 4u) != 0) ? (((mask & 8) == 0) ? $"(0x{dataOffset:X6},0x{dataLength:X4})" : $"(0x{tag:X2},0x{dataOffset:X6},0x{dataLength:X4})") : (((mask & 8) == 0) ? $"({dataOffset},{dataLength})" : $"({tag},{dataOffset},{dataLength})"));
			switch (tag)
			{
			case 3:
